...
<?php endif; ?>                    <th class="tcr"><?= __('Last post') ?></th>
            ...
            foreach ($topic_data as $topic) {
                ...
                ?>
                    ...
                    <td class="tcl">
                        <div class="<?= $topic['icon_type'] ?>"><div class="nosize"><?= Utils::numberFormat($topic_count + $start_from) ?></div></div>
                        <div class="tclcon">
                            <div>
                                <?= $topic['subject_disp']."\n" ?>
                            </div>
                    ...
                    <td class="tc2"><?php echo(!$topic['ghost_topic']) ? Utils::numberFormat($topic['num_replies']) : '-' ?></td>
<?php if (ForumSettings::get('o_topic_views') == '1') :
?>                    <td class="tc3"><?php echo(!$topic['ghost_topic']) ? Utils::numberFormat($topic['num_views']) : '-' ?></td>
<?php endif;
...
<div class="linksb">
    <div class="inbox crumbsplus">
        <div class="pagepost">
            <p class="pagelink conl"><?= $paging_links ?></p>
            <p class="conr modbuttons"><input type="submit" name="move_topics" value="<?= __('Move') ?>"<?= $button_status ?> /> <input type="submit" name="delete_topics" value="<?= __('Delete') ?>"<?= $button_status ?> /> <input type="submit" name="merge_topics" value="<?= __('Merge') ?>"<?= $button_status ?> /> <input type="submit" name="open" value="<?= __('Open') ?>"<?= $button_status ?> /> <input type="submit" name="close" value="<?= __('Close') ?>"<?= $button_status ?> /></p>
            <div class="clearer"></div>
        </div>
        <ul class="crumbs">
            <li><a href="<?= Url::base() ?>"><?= __('Index') ?></a></li>
