...
<?php if (User::get()->id == $id || User::get()->g_id == ForumEnv::get('FEATHER_ADMIN') || ($user->g_moderator == '0' && User::get()->g_mod_change_passwords == '1')): ?>                            <p class="actions"><span><a href="<?= Router::pathFor('profileAction', ['id' => $id, 'action' => 'change_pass']) ?>"><?= __('Change pass') ?></a></span></p>
<?php endif; ?>                        </div>
                </fieldset>
            </div>
            <div class="inform">
...
<?php if (User::get()->is_admmod): ?>                            <label><?= __('Admin note') ?><br />
