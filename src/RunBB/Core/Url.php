...
class Url
{
    ...
    public static function paginate($num_pages, $cur_page, $link, $args = null)
    {
        ...
        if ($num_pages <= 1) {
            ...
            for ($current = ($cur_page == 5) ? $cur_page - 3 : $cur_page - 2,
                 $stop = ($cur_page + 4 == $num_pages) ? $cur_page + 4 : $cur_page + 3; $current < $stop; ++$current) {
    ...
    public static function paginateOld($num_pages, $cur_page, $link)
    {
        ...
        if ($num_pages <= 1) {
            ...
            // Add a previous page link
            if ($num_pages > 1 && $cur_page > 1) {
                $pages[] = '<a rel="prev"'.(empty($pages) ? ' class="item1"' : '')
                    .' href="'.$link.($cur_page == 2 ? '' : '&amp;p='.($cur_page - 1)).'">'.__('Previous').'</a>';
            }
            ...
            // Add a next page link
            if ($num_pages > 1 && !$link_to_all && $cur_page < $num_pages) {
                $pages[] = '<a rel="next"'.(empty($pages) ? ' class="item1"' : '').' href="'.$link.'&amp;p='.
                    ($cur_page +1).'">'.__('Next').'</a>';
            }
        }
