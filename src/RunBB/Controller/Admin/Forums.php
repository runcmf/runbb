        ...
        if (Request::isPost()) {
            if (Input::post('save') && Input::post('read_forum_old')) {
                ...
                $forum_data = ['forum_name' => Utils::escape(Input::post('forum_name')),
                    'forum_desc' => Input::post('forum_desc') ?
                        Utils::linebreaks(Utils::trim(Input::post('forum_desc'))) : null,
                    'cat_id' => (int)Input::post('cat_id'),
                    'sort_by' => (int)Input::post('sort_by'),
                    'redirect_url' => Url::isValidUrl(Input::post('redirect_url')) ?
                        Utils::escape(Input::post('redirect_url')) : null];

                if ($forum_data['forum_name'] == '') {
                    return Router::redirect(
                        Router::pathFor('editForum', ['id' => $args['id']]),
                        __('Must enter name message')
                    );
                }
                if ($forum_data['cat_id'] < 1) {
                    return Router::redirect(
                        Router::pathFor('editForum', ['id' => $args['id']]),
                        __('Must be valid category')
                    );
                }
                ...
                foreach ($permissions as $perm_group) {
                    $permissions_data = ['group_id' => $perm_group['g_id'],
                        'forum_id' => $args['id']];
                    if ($perm_group['g_read_board'] == '1' &&
                        isset(Input::post('read_forum_new')[$perm_group['g_id']]) &&
                        Input::post('read_forum_new')[$perm_group['g_id']] == '1'
                    ) {
                    ...
                    $permissions_data['post_replies'] =
                        (isset(Input::post('post_replies_new')[$perm_group['g_id']])) ? '1' : '0';
                    $permissions_data['post_topics'] =
                        (isset(Input::post('post_topics_new')[$perm_group['g_id']])) ? '1' : '0';
                    ...
                    if ($permissions_data['read_forum'] != Input::post('read_forum_old')[$perm_group['g_id']] ||
                        ...
                        if ($permissions_data['read_forum'] == '1' &&
                            $permissions_data['post_replies'] == $perm_group['g_post_replies'] &&
                            $permissions_data['post_topics'] == $perm_group['g_post_topics']
                        ) {
