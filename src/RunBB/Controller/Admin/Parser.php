...
class Parser
{
    ...
    public function display($req, $res, $args)
    {
        ...
        // Legacy
        require ForumEnv::get('FORUM_ROOT') . 'lang/' . User::get()->language . '/admin/parser.php';
        ...
        if (Input::post('form_sent')) {
                    ...
                    case 0: // 0: Successful upload.
                        ...
                        if (preg_match('/^[\w\-.]++$/', $name)) {            // If we have a valid filename?
                            if (preg_match('%^image/%', $f['type'])) {        // If we have an image file type?
                                ...
                                throw new  RunBBException($lang_admin_parser['upload_err_3'], 400);
                            }
                        } else { // Error #4: 'Smiley upload failed. Bad filename.'
                            throw new  RunBBException($lang_admin_parser['upload_err_4'], 400);
                        }
                        break;
            ...
            require_once('RunBB/Core/parser/bbcd_compile.php'); // Compile $bbcd and save into $pd['bbcd']
